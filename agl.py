...
def sincronizar():
    ...
    try:
        ...
        horas_clientes = [
            hora_para_float(hora)
            for hora in dados["clientes"]
        ]

        quantidade_total = len(horas_clientes) + 1

        media = (
            hora_servidor + sum(horas_clientes)
        ) / quantidade_total
        ...
        ajuste_servidor = diferenca_minutos(
            media,
            hora_servidor
        )
        ...
        eventos_ordenados = sorted(
            eventos,
            key=lambda evento: evento["hora_original"]
        )
